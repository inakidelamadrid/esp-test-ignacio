...
import { changeTeamSelection } from '../../../actions/TeamSelection'

const hydrateTeamSelection = (playerData, teamSelection) => {
  return reduce(
    teamSelection,
    (acc, playerIds, key) => {
      const hydratedPlayers = playerIds.map(playerId =>
        playerId ? find(playerData, { id: playerId }) : null
      )
...
class Formation extends Component {
  ...
  handleFormationSet = (e, data) => {
    ...
    this.props.changeTeamSelection(data.value)
  }

  render() {
    const { formations, players, teamSelection } = this.props
    const siftPlayers = players.map(player =>
      pick(player, ['country', 'id', 'first_name', 'last_name'])
    )
    ...
    const { forwards, midfielders, defenders, keeper } = hydrateTeamSelection(
      siftPlayers,
      teamSelection
    )

    return (
      <Transition animation="fade" duration={300} transitionOnMount>
        <Grid data-component="Formation" padded>
          <Grid.Row>
          ...
          <FormationRowPlayers players={keeper} position={Position[3]} />
        </Grid>
      </Transition>
    )
  }
